...
The two ways a person joins an Equal Split Pool as a Member: an open shareable link, or a six-digit code entered manually — both are the same underlying `joinCode`, and both are gated identically. Since ADR-0019, using either creates a **Join Request**, not an immediate Membership: the Organizer must approve it before the requester becomes a Member and can deposit. The Organizer can also set an expiry on the code/link itself from the Add Member screen's share step; unset by default. Custom Split Pools do not use this mechanism; see **Invitation**.
...
Since ADR-0019, the same entity also backs an Equal Split Organizer explicitly adding one person by phone number or device contact — the one exception to Equal Split's Join Request approval gate (see **Invite Link / Pool Code**, **Join Request**): the Organizer choosing that specific person is itself the approval. This variant carries no assigned amount (Equal Split has no per-invitee share, only a Pool-wide `perPersonAmountPaise`), and accepting it creates a Membership immediately at zero cost rather than requiring a Deposit.
_Avoid_: Invite (too generic — collides with Invite Link/Pool Code, a different mechanism)

**Join Request** (ADR-0019):
An Equal Split Pool joiner's request to join via Invite Link or Pool Code, pending the Organizer's approval. Distinct from a Membership — a Join Request does not grant Pool access, appear in the Member list, or count toward the per-person amount; approving one atomically creates a Membership, the same way paying an Invitation does. A Join Request never expires on its own — it sits pending until the Organizer approves or declines it, however long that takes; the only expiry in this flow is on the Invite Link/Pool Code itself (see that entry), checked once at the moment the join attempt is made. A declined Join Request cannot be immediately retried via the same link — the Organizer can still add that person directly by phone/contact instead. Visible only to the Organizer, on the All Members screen. Does not apply to Custom Split Pools, or to an Organizer's direct phone/contact add (see **Invitation**), which is pre-approved by construction.
_Avoid_: Pending Member, Invite Request (use "Join Request" — the specific term this ADR establishes)
