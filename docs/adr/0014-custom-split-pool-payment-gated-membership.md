...
- Equal Split Pools are unaffected: Invite Link/Pool Code joining stays immediate, and Deposits there remain unenforced against the expected share. Equal Split also now picks up the "ongoing" use case (apartment/roommate expenses) that Open Pool used to serve — a Member there isn't limited to a single Deposit, so repeat top-ups over time work today without any schema change.
